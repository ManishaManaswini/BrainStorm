...
class FeatEx:
    def __init__(self):
        self.freq_bands = np.logspace(*np.log10([1, 35]), num=8)
    ...
    def extract_power_band_ratios(self, epochs):
        ...
        for i, epoch in enumerate(epochs):
            ...
            for channel_idx, channel_name in enumerate(epochs.info['ch_names']):
                ...
                # Compute power ratios for the current channel
                # channel_power['theta_delta_ratio'] = channel_power['theta_power'] / channel_power['delta_power']
                # channel_power['alpha_delta_ratio'] = (channel_power['alpha_power']+ channel_power['low_beta_power'])/ channel_power['delta_power']
                # channel_power['low_beta_delta_ratio'] = channel_power['low_beta_power'] / channel_power['delta_power']
                # channel_power['alpha_theta_ratio'] = (channel_power['alpha_power']+ channel_power['low_beta_power'])/ channel_power['theta_power']
                # channel_power['low_beta_theta_ratio'] = channel_power['low_beta_power'] / channel_power['theta_power']
                # channel_power['low_beta_alpha_ratio'] = channel_power['low_beta_power'] / channel_power['alpha_power']
    ...
    def extract_time_frequency_features(self, epochs, freq_bands):
        ...
        n_cycles = [x / 2. for x in freq_bands]
    ...
    def plot_all_power_band_ratios(self, all_features, parameters):
        ...
        for file, feature in all_features.items():
            ...
            fig.suptitle(f"Power Band Ratios for {file}", fontsize=22, fontweight='bold')
    ...
    def plot_specific_epochs_power_band_ratios(self, all_features, epoch_indices, parameters):
        c = 0
        figs = []
        titles = []
        for i in range(3, len(all_features.keys()), 3):
            ...
            fig, axs = plt.subplots(len(parameters), len(file_names), figsize=(15, 10), sharey='row')

            # Iterate over the file names
            for file_idx, file_name in enumerate(file_names):
                # Get the power band ratios for the current file
                power_band_ratios = all_features[file_name]['power_band_ratios']
                ...
                for param_idx, param in enumerate(parameters):
                    ...
                    for epoch_idx in epoch_indices:
                        ...
                        if epoch_idx not in power_band_ratios:
                            continue

                        # Get the power band ratios for the current epoch
                        epoch_data = power_band_ratios[epoch_idx]

                        # Create a list to store the data for the current parameter and epoch
                        data = []

                        # Iterate over the channel names
                        for ch_idx, ch_name in enumerate(channel_names):
                            # Get the power band ratios for the current channel
                            ch_data = epoch_data[ch_name]

                            # Append the data for the current parameter and channel to the data list
                            data.append(ch_data[param])

                        # Plot the data for the current parameter and epoch
                        axs[param_idx, file_idx].bar(
                            [x + (0.2 * (ch_idx - min(range(len(channel_names))))) - 0.6 + 0.1 + (
                                        0.05 * (epoch_idx - min(epoch_indices))) for x in range(len(channel_names))],
                            data, width=0.05,
                            color=cmap(epoch_idx - min(epoch_indices)))

                        # Set the title of the subplot
                        axs[param_idx, file_idx].set_title(f"{file_name} - {param}")

                        # Set the x-ticks and x-tick labels of the subplot
                        axs[param_idx, file_idx].set_xticks([x + 0.25 for x in range(len(channel_names))])
                        axs[param_idx, file_idx].set_xticklabels(channel_names, rotation=45)

            # Add a legend to the subplot
            axs[0, file_idx].legend([f"Epoch {epoch_idx}" for epoch_idx in epoch_indices])
    ...
    def plot_all_spectral_entropies(self, all_features):
        ...
        for file, feature in all_features.items():
            ...
            # Add a title to the plot
            fig.suptitle(f"Spectral Entropies for {file}", fontsize=22, fontweight='bold')
    ...
    def plot_specific_epochs_spectral_entropy(self, all_features, epoch_indices):
        ...
        for i in range(3, len(all_features.keys()), 3):
            ...
            # Iterate over the file names
            for file_idx, file_name in enumerate(file_names):
                # Get the spectral entropy for the current file
                spectral_entropy = all_features[file_name]['spectral_entropy']

                # Iterate over the epoch indices
                for epoch_idx in epoch_indices:
                    # Check if the current epoch index is valid for the current file
                    if epoch_idx not in spectral_entropy:
                        continue

                    # Get the spectral entropy for the current epoch
                    epoch_data = spectral_entropy[epoch_idx]

                    # Plot the spectral entropy for each channel
                    for ch_idx, ch_name in enumerate(channel_names):
                        # Get the spectral entropy for the current channel
                        se = epoch_data[ch_name]

                        # Create a bar plot for the current channel
                        axs[file_idx].bar(ch_idx + (0.2 * (ch_idx - min(range(len(channel_names))))) - 0.6 + 0.1 + (
                                    0.05 * (epoch_idx - min(epoch_indices))), se, width=0.05,
                                          color=cmap(epoch_idx - min(epoch_indices)))
            ...
            fig.text(0.04, 0.5, 'Spectral Entropy', va='center', rotation='vertical')
    ...
    def plot_all_psd_values(self, all_features, parameters):
        ...
        for file, feature in all_features.items():
            ...
            # Add a title to the plot
            fig.suptitle(f"Power spectral density values for {file}", fontsize=22, fontweight='bold')
    ...
    def plot_all_coherence_values(self, all_features):
        ...
        for file, feature in all_features.items():
            ...
            # Add a title to the plot
            fig.suptitle(f"Coherence Values for {file}", fontsize=22, fontweight='bold')
    ...
    def plot_avg_coherence_features(self, all_features,
                                    channel_pairs=[("AF7_AF8"), ("AF7_TP9"),
                                                                   ("AF7_TP10"), ("AF8_TP9"),
                                                                   ("AF8_TP10"), ("TP9_TP10")]):
        colormap = plt.cm.get_cmap('tab10')
        figs = []
        ...
        for channel_pair in channel_pairs:
            ...
            channel_pair_str = f"{channel_pair[0]}-{channel_pair[1]}"
    ...
    def plot_avg_tfr_heatmap(self, all_features, channel_names=["AF7", "AF8", "TP9", "TP10"]):
        ...
        for file, feature in all_features.items():
            ...
            # Add a title to the plot
            fig.suptitle(f"Time-Frequency Representation for {file}", fontsize=22, fontweight='bold')
            plt.tight_layout(rect=[0, 0, 1, 0.96])  # Adjust the layout to accommodate the title
    ...
    def plot_all_tfr_values(self, all_features, channel_names=["AF7", "AF8", "TP9", "TP10"]):
        ...
        for file, feature in all_features.items():
            ...
            axs[-1].set_xlabel('Time (s)')  # changed to 'Time (s)'

            # Add a title to the plot
            fig.suptitle(f"Average Time-Frequency Representation for {file}", fontsize=22, fontweight='bold')
    ...
    def plot_avg_tfr_values(self, all_features, channel_names=["AF7", "AF8", "TP9", "TP10"]):
        ...
        # Determine the longest time axis
        max_epochs = max([features['total_epochs'] for _, features in all_features.items()])
        time_axis = np.linspace(0, max_epochs * 5, max_epochs)  # Assume each epoch is 5 seconds long

        # Fetching times from one of the TFR objects in all_features
        ...
        for channel_name in channel_names:
            ...
            tfr_data_list = [features['tfr_features'].data[channel_idx] for _, features in all_features.items()]

            # Average the TFR data across the files for each frequency and time point
            avg_tfr_data = np.mean(tfr_data_list, axis=0)

            # Extract frequencies from one of the TFR objects
            ...
            ch_info = create_info([channel_name], sfreq=500,
                                  ch_types=ch_type_str)  # Assuming a sampling frequency of 500 Hz

            # Create an AverageTFR object for the averaged data
            avg_tfr_obj = AverageTFR(info=ch_info,
                                     data=avg_tfr_data[None, ...],  # Add an extra dimension for channels
                                     times=tfr_times,
                                     freqs=freqs,
                                     nave=len(all_features),
    ...
    def plot_all_stat_values(self, all_features, parameters):
        ...
        for file, feature in all_features.items():
            ...
            # Add a title to the plot
            fig.suptitle(f"Stat values for {file}", fontsize=22, fontweight='bold')
    ...
    def plot_all_epoch_stats(self, all_features, parameters, channel_names = ["AF7", "AF8", "TP9", "TP10"]):
        ...
        for file, feature in all_features.items():
            ...
            # Add a title to the plot
            fig.suptitle(f"Epoch Statistics for {file}", fontsize=22, fontweight='bold')
